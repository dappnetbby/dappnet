    ...
    pub async fn handle_client(&mut self) -> Result<usize, MerinoError> {
        ...
        match req.command {
            ...
            SockCommand::Connect => {
                ...
                // Dappnet: perform the port rewrites to the gateway.
                ...
                // if !req.addr.ends_with(b".eth") {
                //     return Err(MerinoError::Socks(ResponseCode::AddrTypeNotSupported));
                // }

                let port2 = match req.port {
                    80 => 10422,
                    443 => 10424,
                    _ => req.port,
                };

                let sock_addr = addr_to_socket(&req.addr_type, &addr2, port2).await?;
